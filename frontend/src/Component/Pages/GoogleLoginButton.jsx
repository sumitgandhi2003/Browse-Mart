...
/**
 * GoogleLoginButton
 * Reusable button to initiate Google OAuth flow.
 * Props:
...
const GoogleLoginButton = ({ label = "Continue with Google", className = "" }) => {
  ...
  const handleGoogleLogin = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get(`${SERVER_URL}/api/auth/google`);
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className={`flex w-full items-center justify-center gap-3 rounded-xl border-2 py-3 font-medium transition-all duration-200 hover:shadow-md active:scale-[0.98] disabled:opacity-60 ${className}`}
    >
      {isLoading ? (
